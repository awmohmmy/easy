...
# ===================== CONFIG =====================
PORT = "COM3"          # <-- แก้ให้ตรง ESP32 (Device Manager)
BAUD = 115200

MODEL_PATH = "yolo11l.pt"      # แนะนำ s/l แล้วแต่เครื่อง
LANE_A_DIR = "a"
LANE_B_DIR = "b"
...
STEP_DELAY_SEC = 10            # หน่วงต่อรอบ
SWITCH_MARGIN = 1              # ต่างกันกี่คันถึงสลับ (กันแกว่ง)
SHOW_WINDOWS = True            # โชว์ภาพตีกรอบไหม
SEND_ONLY_ON_CHANGE = True     # กันสแปม serial
# ==================================================
...
def open_serial():
    ...
    time.sleep(2)  # รอ ESP32 รีเซ็ตหลังเปิดพอร์ต
    # เคลียร์บัฟเฟอร์
...
def decide(count_a: int, count_b: int):
    # กันแกว่ง: ต้องต่างกันอย่างน้อย SWITCH_MARGIN ถึงสลับ
...
def main():
    ...
    if not files_a or not files_b:
        ...
        print("เช็กว่าโฟลเดอร์อยู่ข้างไฟล์ .py และมีรูป .jpg/.png")
        return
    ...
    try:
        while True:
            ...
            # === สั่ง NeoPixel วงเดียว ===
            # A มากกว่า -> GREEN, B มากกว่า -> RED, เท่ากัน -> ไม่เปลี่ยน (หรือ OFF ก็ได้)
            if pref == "A":
                cmd = "GREEN"
            elif pref == "B":
                cmd = "RED"
            else:
                cmd = None  # รถใกล้เคียงกัน ไม่เปลี่ยนไฟ

            print(f"[DECIDE] A={count_a}, B={count_b} -> {cmd if cmd else 'HOLD'}")
